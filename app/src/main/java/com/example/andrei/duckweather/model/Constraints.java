package com.example.andrei.duckweather.model;

public final class Constraints {
    public static final String KEY_LATITUDE_REQUEST_LOCATION = "LATITUDE_REQUEST_LOCATION";
    public static final String KEY_LONGITUDE_REQUEST_LOCATION = "LONGITUDE_REQUEST_LOCATION";
    public static final String KEY_LOCATION_REQUEST_LOCATION = "LOCATION_REQUEST_LOCATION";
    public static final String KEY_LOCATION_SHARED_PREFERENCES = "LOCATION_SHARED_PREFERENCES";
    public static final String KEY_LATITUDE_SHARED_PREFERENCES = "LATITUDE_SHARED_PREFERENCES";
    public static final String KEY_LONGITUDE_SHARED_PREFERENCES = "LONGITUDE_SHARED_PREFERENCES";

}
